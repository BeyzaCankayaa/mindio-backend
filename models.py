...
class Suggestion(Base):
    ...
    # DB'de sen bunu NOT NULL + DEFAULT false yaptın -> burada da uyumlu olsun
...
class User(Base):
    ...
    # FIX: UserProfile relationship
    profiles = relationship("UserProfile", back_populates="user", lazy="selectin")

    # (optional but useful) reward claims relationship
...
class Suggestion(Base):
    ...
    # DB'de NOT NULL + DEFAULT false -> burada uyumlu
    is_approved = Column(Boolean, nullable=False, server_default="false")
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
